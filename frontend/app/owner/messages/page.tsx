...
const formatConversation = (convo: any) => {
  ...
  return {
    ...
    unit: otherUser?.unit?.unit_number || 'N/A',
...
export default function MessagesPage() {
  ...
  const messages = (thread && conversations.find(c => String(c.id) === activeThread)?.messages) || [];
      ...
      <header className="navbar">
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          ...
          <div>
            ...
            <div style={{fontSize:11,color:"#4a6080"}}>{displayThreads.filter(t=>t.unread>0).length} unread conversations</div>
          </div>
        </div>
