...
type PayStatus = "paid"|"pending"|"overdue"|"completed"|"failed";
...
const formatPayment = (payment: Payment) => {
  const initials = payment.tenant?.name?.split(' ').map(n => n[0]).join('') || 'UN';
  ...
  const displayStatus: PayStatus = payment.status === 'completed' ? 'paid' : payment.status === 'failed' ? 'overdue' : 'pending';
  ...
  return {
    ...
    date: new Date(payment.payment_date).toLocaleDateString('en-IN', { month: 'short', day: 'numeric' }),
    month: new Date(payment.payment_date).toLocaleDateString('en-IN', { month: 'short', year: 'numeric' }),
    grad: gradients[payment.id % gradients.length],
...
export default function PaymentsPage() {
  ...
  const displayPayments = payments.map(formatPayment);
  ...
  const totalCollected = displayPayments.filter(p=>p.status==="paid").reduce((a,p)=>a+parseInt(p.amount.replace(/[₹,]/g,"")),0);
  const totalPending   = displayPayments.filter(p=>p.status==="pending").reduce((a,p)=>a+parseInt(p.amount.replace(/[₹,]/g,"")),0);
  const totalOverdue   = displayPayments.filter(p=>p.status==="overdue").reduce((a,p)=>a+parseInt(p.amount.replace(/[₹,]/g,"")),0);
      ...
      <main className="page-main">
        ...
        {loading && (
          <div style={{textAlign:"center",padding:"60px 20px",color:"#4a6080"}}>
            ...
            <div style={{fontSize:14,color:"#94a3b8"}}>Loading payments...</div>
          </div>
        )}
          ...
          <>
            ...
            <div className="a1" style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:14,marginBottom:22}}>
              <div>
                ...
                <div style={{fontSize:13,color:"#94a3b8",marginTop:3}}>March 2026 · {displayPayments.length} transactions</div>
              </div>
