...
import { paymentAPI, maintenanceAPI, leaseAPI, authAPI } from "@/app/services/api";
...
export default function TenantDashboardPage() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        setLoading(true);
        const [profileRes, paymentsRes, maintenanceRes, leaseRes] = await Promise.all([
          authAPI.profile().catch(() => ({ data: {} })),
          paymentAPI.getAll().catch(() => ({ data: [] })),
          maintenanceAPI.getAll().catch(() => ({ data: [] })),
          leaseAPI.getAll().catch(() => ({ data: [] }))
        ]);
        
        setProfile(profileRes.data || {});
        setPayments(paymentsRes.data || []);
        setMaintenance(maintenanceRes.data || []);
        const userLeases = leaseRes.data || [];
        setLease(userLeases.length > 0 ? userLeases[0] : null);
        setError(null);
      } catch (err: any) {
        ...
        <div style={{ margin:"14px 12px", background:"rgba(6,182,212,0.06)", border:"1px solid rgba(6,182,212,0.15)", borderRadius:12, padding:"14px" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <div style={{ overflow:"hidden" }}>
              <div style={{ fontSize:13, fontWeight:700, color:"#e2e8f0", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{profile?.name || 'Loading...'}</div>
  ...
  <span>Logout</span>
      ...
      <main className="page-main">
        ...
        <div className="a1" style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:14, marginBottom:20 }}>
          <div>
            <div style={{ fontSize:"clamp(18px,2.5vw,24px)", fontWeight:800, color:"#e2e8f0", letterSpacing:"-0.02em" }}>
              Good morning, {profile?.name?.split(" ")[0] || 'Tenant'} 👋
            </div>
        ...
        {(() => {
          ...
          return (
            <div className="a2" style={{display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(160px, 1fr))", gap:14, marginBottom:20}}>
              <div className="stat-card" style={{borderColor: pendingPayment ? '#f59e0b' : '#1e2d47'}}>
                <div style={{width:38, height:38, borderRadius:10, background:"rgba(245,158,11,0.1)", display:"grid", placeItems:"center", fontSize:18}}>💳</div>
                <div style={{fontSize:"clamp(18px,2vw,26px)", fontWeight:800, color:"#e2e8f0", letterSpacing:"-0.02em", marginTop:12}}>₹{pendingPayment?.amount?.toLocaleString() || '0'}</div>
                <div style={{fontSize:12, color:"#f59e0b", fontWeight:600, marginTop:4}}>Rent Due</div>
                <div style={{fontSize:11, color:"#4a6080", marginTop:2}}>Due {pendingPayment?.due_date ? new Date(pendingPayment.due_date).toLocaleDateString('en-IN', {month:'short', day:'numeric'}) : 'N/A'}</div>
              </div>
              <div className="stat-card">
                <div style={{width:38, height:38, borderRadius:10, background:"rgba(6,182,212,0.1)", display:"grid", placeItems:"center", fontSize:18}}>📋</div>
                <div style={{fontSize:"clamp(18px,2vw,26px)", fontWeight:800, color:"#e2e8f0", letterSpacing:"-0.02em", marginTop:12}}>{lease?.months_remaining || '0'}</div>
                <div style={{fontSize:12, color:"#06b6d4", fontWeight:600, marginTop:4}}>Lease Months</div>
                <div style={{fontSize:11, color:"#4a6080", marginTop:2}}>Left in lease</div>
              </div>
              <div className="stat-card" style={{borderColor: openMaintenance > 0 ? '#f43f5e' : '#1e2d47'}}>
                <div style={{width:38, height:38, borderRadius:10, background:"rgba(244,63,94,0.1)", display:"grid", placeItems:"center", fontSize:18}}>🔧</div>
                <div style={{fontSize:"clamp(18px,2vw,26px)", fontWeight:800, color:"#e2e8f0", letterSpacing:"-0.02em", marginTop:12}}>{openMaintenance}</div>
                <div style={{fontSize:12, color:"#f43f5e", fontWeight:600, marginTop:4}}>Open Requests</div>
                <div style={{fontSize:11, color:"#4a6080", marginTop:2}}>{openMaintenance > 0 ? 'Needs action' : 'All resolved'}</div>
              </div>
            </div>
          );
        })()}
              ...
              <table style={{ width:"100%", borderCollapse:"collapse" }}>
                ...
                <tbody>
                  {(payments.slice(0, 5) || []).map((p: any, i: number) => (
                    <tr key={i} className="trow" style={{ borderBottom: i < Math.min(payments.length - 1, 4) ? "1px solid rgba(30,45,71,0.5)" : "none" }}>
                      <td style={{ padding:"10px 0", fontSize:13, color:"#e2e8f0", fontWeight:500 }}>{new Date(p.created_at).toLocaleDateString('en-IN', {month:'short', year:'2-digit'})}</td>
                      <td style={{ padding:"10px 0", fontSize:13, fontWeight:700, color:"#e2e8f0" }}>₹{p.amount.toLocaleString()}</td>
                      <td style={{ padding:"10px 0" }}><Pill {...PAY_MAP[p.status as PayStatus]} /></td>
                      <td style={{ padding:"10px 0", fontSize:12, color: p.receipt_id ? "#06b6d4" : "#4a6080" }}>
                        {p.receipt_id || "—"}
                      </td>
                    </tr>
                  ))}
        ...
        <div className="three-col a5" style={{ marginBottom:18 }}>
          ...
          <div className="glass-card">
            ...
            {(maintenance.slice(0, 3) || []).map((m: any, i: number) => (
              <div key={i} className="maint-row">
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:6 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    <div style={{ width:4, height:20, borderRadius:2, background:REQ_MAP[m.status as ReqStatus].color, flexShrink:0 }} />
                    <div>
                      <div style={{ fontSize:13, fontWeight:600, color:"#e2e8f0", lineHeight:1.3 }}>{m.title}</div>
                      <div style={{ fontSize:11, color:"#4a6080" }}>{m.category} · MR-{String(m.id).padStart(3,'0')}</div>
                    </div>
                  </div>
                  <Pill {...REQ_MAP[m.status as ReqStatus]} />
                </div>
                <div style={{ fontSize:11, color:"#4a6080", paddingLeft:12 }}>
                  📅 {new Date(m.created_at).toLocaleDateString('en-IN')} &nbsp;·&nbsp; {m.description?.substring(0, 30)}...
                </div>
              </div>
            ))}
