...
export default function RegisterPage() {
  ...
  const handleRegister = () => {
    ...
    setTimeout(() => {
      // Save user to localStorage (same shape as login)
      const user = { name, email, phone, role };
      localStorage.setItem("rentmanager_user", JSON.stringify(user));

      // Set role cookie so middleware can protect routes
      document.cookie = `role=${role}; path=/; max-age=${60 * 60 * 24 * 30}`; // 30 days
    ...
    <>
      ...
      <div style={{position:"relative",zIndex:1,minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px",overflowY:"auto"}}>
        ...
        <div className="card">
          ...
          {/* Error */}
          {error && (
            <div style={{marginTop:14,padding:"10px 14px",background:"rgba(244,63,94,0.08)",border:"1px solid rgba(244,63,94,0.2)",borderRadius:9,fontSize:13,color:"#f43f5e"}}>
              ⚠️ {error}
            </div>
          )}

          {/* Submit */}
          <button className="submit-btn" onClick={handleRegister} disabled={loading} style={{marginTop:18,background:role==="owner"?"linear-gradient(135deg,#8b5cf6,#6366f1)":"linear-gradient(135deg,#06b6d4,#8b5cf6)"}}>
