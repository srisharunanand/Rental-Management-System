...
export default function SettingsPage() {
  ...
  const [accountStats, setAccountStats] = useState({ propertiesManaged:"5", totalTenants:"56", activeLeases:"12", monthlyRevenue:"₹90,000", memberSince:"Jan 2022" });
        ...
        {tab==="Profile"&&(
          <div className="a3 settings-grid">
            ...
            <div className="glass-card">
              ...
              {[
                {icon:"🏠",label:"Properties Managed",key:"propertiesManaged"},
                {icon:"👥",label:"Total Tenants",      key:"totalTenants"},
                {icon:"📋",label:"Active Leases",      key:"activeLeases"},
                {icon:"💰",label:"Monthly Revenue",    key:"monthlyRevenue"},
                {icon:"📅",label:"Member Since",       key:"memberSince"},
              ].map(s=>(
                <div key={s.label} style={{display:"flex",justifyContent:"space-between",padding:"12px 0",borderBottom:"1px solid rgba(30,45,71,0.5)",fontSize:13}}>
                  ...
                  <span style={{fontWeight:700,color:"#e2e8f0"}}>{accountStats[s.key as keyof typeof accountStats]}</span>
                </div>
              ))}
