...
const INVOICES: {
  id:string; tenant:string; initials:string; unit:string; property:string;
  amount:string; status:InvStatus; date:string; due:string; grad:string; items:string[];
}[] = [
  { id:"INV-0032", tenant:"Rahul Krishnan", initials:"RK", unit:"Unit 4B", property:"Greenview Apt",  amount:"₹18,000", status:"paid",    date:"Mar 1",  due:"Mar 5",  grad:"linear-gradient(135deg,#06b6d4,#8b5cf6)", items:["March Rent — ₹18,000"] },
  { id:"INV-0031", tenant:"Priya Mehta",    initials:"PM", unit:"Unit 2A", property:"Sunrise Villas", amount:"₹22,000", status:"sent",    date:"Mar 1",  due:"Mar 5",  grad:"linear-gradient(135deg,#f59e0b,#ef4444)", items:["March Rent — ₹22,000"] },
  { id:"INV-0030", tenant:"Suresh Sharma",  initials:"SS", unit:"Unit 1C", property:"Palm Heights",   amount:"₹15,500", status:"overdue", date:"Feb 28", due:"Feb 28", grad:"linear-gradient(135deg,#ef4444,#ec4899)", items:["February Rent — ₹15,000","Late Fee — ₹500"] },
  { id:"INV-0029", tenant:"Anjali Nair",    initials:"AN", unit:"Unit 3D", property:"Greenview Apt",  amount:"₹20,000", status:"paid",    date:"Mar 1",  due:"Mar 5",  grad:"linear-gradient(135deg,#22c55e,#06b6d4)", items:["March Rent — ₹20,000"] },
  { id:"INV-0028", tenant:"Divya Varma",    initials:"DV", unit:"Unit 5A", property:"Palm Heights",   amount:"₹17,500", status:"paid",    date:"Mar 1",  due:"Mar 5",  grad:"linear-gradient(135deg,#8b5cf6,#6366f1)", items:["March Rent — ₹17,500"] },
  { id:"INV-0027", tenant:"Karthik Raj",    initials:"KR", unit:"Unit 6B", property:"Royal Enclave",  amount:"₹30,000", status:"draft",   date:"—",      due:"Mar 5",  grad:"linear-gradient(135deg,#06b6d4,#22c55e)", items:["March Rent — ₹30,000"] },
];
      ...
      <main className="page-main">
        <div className="a1" style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:14,marginBottom:22}}>
          <div>
            ...
            <div style={{fontSize:13,color:"#94a3b8",marginTop:3}}>{INVOICES.length} invoices · March 2026</div>
          </div>
        ...
        <div className="a2" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:12,marginBottom:20}}>
          {[
            {icon:"🧾",label:"Total",   value:INVOICES.length,                                   color:"#06b6d4",bg:"rgba(6,182,212,0.1)"},
            {icon:"✅",label:"Paid",    value:INVOICES.filter(i=>i.status==="paid").length,      color:"#22c55e",bg:"rgba(34,197,94,0.1)"},
            {icon:"📤",label:"Sent",    value:INVOICES.filter(i=>i.status==="sent").length,      color:"#06b6d4",bg:"rgba(6,182,212,0.1)"},
            {icon:"⚠️",label:"Overdue", value:INVOICES.filter(i=>i.status==="overdue").length,   color:"#f43f5e",bg:"rgba(244,63,94,0.1)"},
          ].map(s=>(
