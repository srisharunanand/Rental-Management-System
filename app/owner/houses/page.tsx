...
type HouseStatus = "fully-occupied" | "partially-occupied" | "vacant";

const HOUSES: {
  id: string; icon: string; name: string; address: string; type: string;
  units: number; occupied: number; rent: string; status: HouseStatus;
  amenities: string[];
}[] = [
  { id:"H001", icon:"🏢", name:"Greenview Apartments", address:"12, Anna Nagar, Chennai – 600 040", type:"Apartment Complex", units:12, occupied:11, rent:"₹18,000/mo", status:"fully-occupied",      amenities:["🅿️ Parking","🏋️ Gym","🔒 CCTV"] },
  { id:"H002", icon:"🏠", name:"Sunrise Villas",        address:"45, Velachery Main Rd, Chennai",   type:"Independent Villa",  units: 8, occupied: 6, rent:"₹22,000/mo", status:"partially-occupied", amenities:["🌳 Garden","🅿️ Parking","💧 Borewell"] },
  { id:"H003", icon:"🏗️", name:"Palm Heights",          address:"78, Tambaram West, Chennai",       type:"Apartment Complex", units:10, occupied: 9, rent:"₹15,000/mo", status:"fully-occupied",      amenities:["🔒 CCTV","🅿️ Parking","🏊 Pool"] },
  { id:"H004", icon:"🏡", name:"Blue Ridge Homes",       address:"22, Porur, Chennai – 600 116",     type:"Row House",          units: 6, occupied: 0, rent:"₹25,000/mo", status:"vacant",              amenities:["🌳 Garden","🅿️ Parking"] },
  { id:"H005", icon:"🏛️", name:"Royal Enclave",          address:"9, Adyar, Chennai – 600 020",      type:"Apartment Complex", units:20, occupied:15, rent:"₹30,000/mo", status:"partially-occupied", amenities:["🏋️ Gym","🏊 Pool","🔒 CCTV","🅿️ Parking"] },
];

const STATUS_MAP: Record<HouseStatus, { bg: string; color: string; label: string }> = {
  "fully-occupied":      { bg:"rgba(34,197,94,0.1)",  color:"#22c55e", label:"● Fully Occupied"  },
  "partially-occupied":  { bg:"rgba(245,158,11,0.1)", color:"#f59e0b", label:"◐ Partially Occupied" },
  "vacant":              { bg:"rgba(244,63,94,0.1)",  color:"#f43f5e", label:"○ Vacant"           },
};
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .house-card{background:#111d33;border:1px solid #1e2d47;border-radius:14px;padding:20px;transition:border-color .2s,transform .2s;cursor:pointer}
        ...
        .modal-bg{position:fixed;inset:0;background:rgba(0,0,0,0.7);backdrop-filter:blur(4px);z-index:500;display:flex;align-items:center;justify-content:center;padding:16px;animation:fadeUp .2s ease}
      ...
      {showModal && (
        <div className="modal-bg" onClick={()=>setShowModal(false)}>
          <div className="modal-card" onClick={e=>e.stopPropagation()}>
            ...
            <div style={{fontSize:18,fontWeight:800,color:"#e2e8f0",marginBottom:4}}>Add New House</div>
            <div style={{fontSize:12,color:"#4a6080",marginBottom:22}}>Fill in the property details below</div>
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              {[
                {label:"Property Name",    placeholder:"e.g. Greenview Apartments"},
                {label:"Full Address",     placeholder:"Street, Area, City – PIN"},
                {label:"Property Type",    placeholder:"Apartment / Villa / Row House"},
                {label:"Number of Units",  placeholder:"e.g. 12"},
              ...
              <div>
                <div style={{fontSize:11,fontWeight:600,color:"#94a3b8",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:6}}>Amenities</div>
                <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                  {["🅿️ Parking","🏋️ Gym","🏊 Pool","🔒 CCTV","🌳 Garden","💧 Borewell"].map(a=>(
                    <label key={a} style={{display:"flex",alignItems:"center",gap:6,padding:"6px 12px",background:"rgba(6,182,212,0.06)",border:"1px solid rgba(6,182,212,0.15)",borderRadius:8,cursor:"pointer",fontSize:12,color:"#94a3b8"}}>
                      <input type="checkbox" style={{accentColor:"#06b6d4"}}/>{a}
                    </label>
                  ))}
                </div>
              </div>
              <button className="add-btn" style={{marginTop:6,width:"100%",padding:"12px"}}>
                Save Property →
              </button>
            </div>
          </div>
      ...
      <main className="page-main">
        ...
        <div className="a1" style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:14,marginBottom:22}}>
          <div>
            ...
            <div style={{fontSize:13,color:"#94a3b8",marginTop:3}}>{HOUSES.length} properties · {HOUSES.reduce((a,h)=>a+h.occupied,0)} tenants</div>
          </div>
        ...
        <div className="a2" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:12,marginBottom:20}}>
          {[
            {icon:"🏘️",label:"Total Properties",value:HOUSES.length,          color:"#06b6d4",bg:"rgba(6,182,212,0.1)"},
            {icon:"🔑",label:"Total Units",      value:HOUSES.reduce((a,h)=>a+h.units,0),    color:"#8b5cf6",bg:"rgba(139,92,246,0.1)"},
            {icon:"👥",label:"Occupied",         value:HOUSES.reduce((a,h)=>a+h.occupied,0), color:"#22c55e",bg:"rgba(34,197,94,0.1)"},
            {icon:"🔓",label:"Vacant Units",     value:HOUSES.reduce((a,h)=>a+(h.units-h.occupied),0), color:"#f43f5e",bg:"rgba(244,63,94,0.1)"},
          ].map(s=>(
        ...
        {/* Filters */}
        <div className="a3" style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:20}}>
          {(["all","fully-occupied","partially-occupied","vacant"] as const).map(f=>(
            <button key={f} className={`filter-btn${filter===f?" active":""}`} onClick={()=>setFilter(f)}>
              {f==="all"?"All Properties":f==="fully-occupied"?"✅ Fully Occupied":f==="partially-occupied"?"⚠️ Partial":"🔴 Vacant"}
            </button>
          ))}
        ...
        <div className={`houses-grid a4`}>
          {filtered.map((h,i)=>{
            const occPct = Math.round((h.occupied/h.units)*100);
            ...
            return (
              <div key={h.id} className="house-card">
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    <div style={{width:42,height:42,borderRadius:11,background:"rgba(6,182,212,0.1)",border:"1px solid rgba(6,182,212,0.2)",display:"grid",placeItems:"center",fontSize:20,flexShrink:0}}>{h.icon}</div>
                    <div>
                      <div style={{fontSize:14,fontWeight:700,color:"#e2e8f0"}}>{h.name}</div>
                      <div style={{fontSize:11,color:"#4a6080"}}>{h.type} · {h.id}</div>
                    </div>
                  </div>
                ...
                <div style={{fontSize:12,color:"#64748b",marginBottom:14,display:"flex",gap:6,alignItems:"flex-start"}}>
                  <span style={{flexShrink:0}}>📍</span>{h.address}
                </div>
                ...
                <div style={{marginBottom:14}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                    ...
                    <span style={{fontSize:12,fontWeight:700,color:"#e2e8f0"}}>{h.occupied}/{h.units} units</span>
                  </div>
                  <div style={{height:6,background:"#1e2d47",borderRadius:4,overflow:"hidden"}}>
                    <div style={{height:"100%",width:`${occPct}%`,background:occPct===100?"linear-gradient(90deg,#22c55e,#06b6d4)":occPct===0?"#f43f5e":"linear-gradient(90deg,#f59e0b,#06b6d4)",borderRadius:4}}/>
                  </div>
                  <div style={{fontSize:11,color:occPct===100?"#22c55e":occPct===0?"#f43f5e":"#f59e0b",marginTop:4,fontWeight:600}}>{occPct}% occupied</div>
                </div>

                {/* Rent + amenities */}
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                  <div>
                    <div style={{fontSize:11,color:"#4a6080"}}>Monthly Rent</div>
                    <div style={{fontSize:16,fontWeight:800,color:"#06b6d4"}}>{h.rent}</div>
                  </div>
                ...
                <div style={{display:"flex",gap:8,paddingTop:14,borderTop:"1px solid #1e2d47"}}>
                  {["✏️ Edit","👥 Tenants","🗑️ Delete"].map((a,i)=>(
                    <button key={a} style={{flex:1,padding:"7px 0",fontSize:11,fontWeight:600,borderRadius:8,border:"1px solid #1e2d47",background:i===2?"rgba(244,63,94,0.06)":"rgba(6,182,212,0.05)",color:i===2?"#f43f5e":"#94a3b8",cursor:"pointer",fontFamily:"inherit",transition:"all .15s"}}
                      onMouseEnter={e=>(e.currentTarget as HTMLButtonElement).style.borderColor=i===2?"rgba(244,63,94,0.4)":"rgba(6,182,212,0.4)"}
        ...
        {filtered.length === 0 && (
          <div style={{textAlign:"center",padding:"60px 20px",color:"#4a6080"}}>
            ...
            <div style={{fontSize:16,fontWeight:600,color:"#94a3b8"}}>No properties found</div>
