...
import { analyticsAPI, propertyAPI, paymentAPI } from "@/app/services/api";
...
const formatAnalytics = async () => {
  try {
    const [analyticsRes, propertyRes] = await Promise.all([
      analyticsAPI.getAll().catch(() => ({ data: [] })),
      propertyAPI.getAll().catch(() => ({ data: [] }))
    ]);
    
    const monthlyData = analyticsRes.data?.monthly || 
      [
        { month:"Sep", income:78500, target:82000, occupancy:85 },
        { month:"Oct", income:82000, target:82000, occupancy:88 },
        { month:"Nov", income:79500, target:8
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      try {
        setLoading(true);
        const data = await formatAnalytics();
  ...
  const maxIncome = Math.max(...monthly.map(m=>m.target)) || 100000;
      ];
    
    const gradients = ['#06b6d4', '#8b5cf6', '#22c55e', '#f59e0b'];
    const propertyData = (propertyRes.data || []).map((p: any, idx: number) => ({
      name: p.property_name,
      units: p.total_units || 0,
      occupied: p.occupied_units || 0,
      income: `₹${((p.occupied_units || 0) * 30000).toLocaleString()}`,
      color: gradients[idx % gradients.length],
    }));
    
    return { monthly: monthlyData, properties: propertyData || [] };
  } catch {
    return { 
      monthly: [
        { month:"Sep", income:78500, target:82000, occupancy:85 },
