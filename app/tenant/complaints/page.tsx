...
export default function TenantComplaintsPage() {
  ...
  const [expanded,    setExpanded]    = useState<string|null>("MR-042");
          ...
          <div className="a3">
            {MY_COMPLAINTS.map(c=>(
              <div key={c.id} className="comp-item">
                {/* Header */}
                <div className="comp-header" style={{padding:16,cursor:"pointer",display:"flex",alignItems:"flex-start",gap:12,justifyContent:"space-between"}} onClick={()=>setExpanded(expanded===c.id?null:c.id)}>
                  <div style={{display:"flex",gap:12,alignItems:"flex-start",flex:1}}>
                    <div style={{width:4,height:40,borderRadius:2,background:PRI_MAP[c.priority].color,flexShrink:0,marginTop:2}}/>
                    <div style={{flex:1}}>
                      <div style={{fontSize:14,fontWeight:600,color:"#e2e8f0",lineHeight:1.3}}>{c.title}</div>
                      <div style={{fontSize:11,color:"#4a6080",marginTop:3}}>{c.id} · {c.category} · {c.date}</div>
                    </div>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
                    <span style={{fontSize:10,fontWeight:600,padding:"3px 9px",borderRadius:20,background:STATUS_MAP[c.status].bg,color:STATUS_MAP[c.status].color,whiteSpace:"nowrap"}}>{STATUS_MAP[c.status].label}</span>
                    <span style={{color:"#4a6080",fontSize:12,transition:"transform .2s",display:"inline-block",transform:expanded===c.id?"rotate(180deg)":"rotate(0deg)"}}>▾</span>
                  </div>
                </div>
                ...
                {expanded===c.id&&(
                  <div style={{padding:"0 16px 16px",borderTop:"1px solid #1e2d47"}}>
                    <div style={{fontSize:13,color:"#94a3b8",lineHeight:1.5,marginBottom:14,marginTop:12}}>{c.note}</div>
                    <div style={{fontSize:11,fontWeight:700,color:"#4a6080",textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:10}}>Updates Timeline</div>
                    {c.updates.map((u,i)=>(
                      <div key={i} style={{display:"flex",gap:10,marginBottom:10}}>
                        <div style={{display:"flex",flexDirection:"column",alignItems:"center"}}>
                          <div style={{width:7,height:7,borderRadius:"50%",background:i===c.updates.length-1?"#06b6d4":"#1e2d47",border:`2px solid ${i===c.updates.length-1?"#06b6d4":"#2d4060"}`,flexShrink:0,marginTop:3}}/>
                          {i<c.updates.length-1&&<div style={{width:1,flex:1,background:"#1e2d47",marginTop:3}}/>}
                        </div>
                        <div style={{paddingBottom:i<c.updates.length-1?6:0}}>
                          <div style={{fontSize:12,color:i===c.updates.length-1?"#e2e8f0":"#94a3b8"}}>{u}</div>
                        </div>
                      </div>
                    ))}
                    {c.status!=="resolved"&&(
