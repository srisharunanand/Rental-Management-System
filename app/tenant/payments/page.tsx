...
export default function TenantPaymentsPage() {
  ...
  const onTimeRate  = Math.round((paidCount/HISTORY.length)*100);
            ...
            <div className="table-wrap">
              ...
              {HISTORY.map(h=>(
                <div key={h.id} className="hist-row">
                  <div>
                    <div style={{fontSize:13,fontWeight:600,color:"#e2e8f0"}}>{h.month}</div>
                    <div style={{fontSize:11,color:"#4a6080"}}>{h.id}</div>
                  </div>
                  <div style={{fontSize:13,fontWeight:700,color:"#e2e8f0"}}>{h.amount}</div>
                  <div style={{fontSize:12,color:"#64748b"}}>{h.method}</div>
                  <div style={{fontSize:12,color:"#64748b"}}>{h.date}</div>
                  <span style={{display:"inline-flex",alignItems:"center",padding:"3px 9px",borderRadius:20,fontSize:11,fontWeight:600,background:PAY_MAP[h.status].bg,color:PAY_MAP[h.status].color,whiteSpace:"nowrap"}}>{PAY_MAP[h.status].label}</span>
                  <button style={{padding:"5px 9px",background:"rgba(6,182,212,0.08)",border:"1px solid rgba(6,182,212,0.2)",borderRadius:7,color:h.status==="paid"?"#06b6d4":"#4a6080",fontSize:11,fontWeight:600,cursor:h.status==="paid"?"pointer":"default",fontFamily:"inherit",opacity:h.status==="paid"?1:0.4,whiteSpace:"nowrap"}}>
                    {h.status==="paid"?"🧾 Receipt":h.status==="pending"?"💸 Pay":"—"}
                  </button>
                </div>
              ))}
            </div>
