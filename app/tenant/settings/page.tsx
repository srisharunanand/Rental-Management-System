...
export default function TenantSettingsPage() {
  ...
  const [leaseData, setLeaseData] = useState({ unitNumber:"Unit 4B", propertyName:"Greenview Apartments", address:"12, Anna Nagar, Chennai – 600 040", leaseStartDate:"January 2024", leaseEndDate:"December 2026", monthlyRent:"₹18,000", depositPaid:"₹36,000" });
  const [preferences, setPreferences] = useState({ currency:"INR (₹)", dateFormat:"DD/MM/YYYY", timeZone:"IST (UTC+5:30)", language:"English" });
    ...
    <>
      ...
      <header className="navbar">
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div style={{width:36,height:36,borderRadius:8,background:"linear-gradient(135deg,#06b6d4,#8b5cf6)",display:"grid",placeItems:"center",color:"white",fontSize:12,fontWeight:700,border:"1.5px solid rgba(6,182,212,0.3)"}}>{profile.name?.split(' ').map((n:string)=>n[0]).join('')||'U'}</div>
        </div>
      </header>
          ...
          <div className="a3 two-col">
            <div className="glass-card">
              ...
              <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:24}}>
                <div style={{width:64,height:64,borderRadius:16,background:"linear-gradient(135deg,#06b6d4,#8b5cf6)",display:"grid",placeItems:"center",color:"white",fontSize:24,fontWeight:700,flexShrink:0}}>{profile.name?.split(' ').map((n:string)=>n[0]).join('')||'U'}</div>
                <div>
                  <div style={{fontSize:16,fontWeight:700,color:"#e2e8f0"}}>{profile.name||'Loading...'}</div>
                  <div style={{fontSize:12,color:"#4a6080"}}>{leaseData.unitNumber} · {leaseData.propertyName}</div>
              ...
              {[
                ...
                <div key={f.label} style={{marginBottom:14}}>
                  ...
                  <input className="inp" value={profile[f.key as keyof typeof profile]} onChange={e=>setProfile({...profile,[f.key]:e.target.value})} placeholder={f.ph}/>
                </div>
              ))}
            ...
            <div className="glass-card">
              ...
              {[
                {icon:"🏠",label:"Unit",          key:"unitNumber"},
                {icon:"🏢",label:"Property",      key:"propertyName"},
                {icon:"📍",label:"Address",       key:"address"},
                {icon:"📅",label:"Tenant Since",  key:"leaseStartDate"},
                {icon:"📋",label:"Lease Ends",    key:"leaseEndDate"},
                {icon:"💰",label:"Monthly Rent",  key:"monthlyRent"},
                {icon:"🏦",label:"Deposit Paid",  key:"depositPaid"},
              ].map(s=>(
                <div key={s.label} style={{display:"flex",justifyContent:"space-between",padding:"11px 0",borderBottom:"1px solid rgba(30,45,71,0.5)",fontSize:13,gap:12}}>
                  ...
                  <span style={{fontWeight:600,color:"#e2e8f0",textAlign:"right"}}>{leaseData[s.key as keyof typeof leaseData]}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="a3 two-col">
            ...
            <div className="glass-card">
              ...
              {[
                ...
                <div key={f.label} style={{marginBottom:14}}>
                  <div style={{fontSize:11,fontWeight:600,color:"#94a3b8",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:6}}>{f.label}</div>
                  <input className="inp" value={preferences[f.key as keyof typeof preferences]} onChange={e=>setPreferences({...preferences,[f.key]:e.target.value})}/>
                </div>
              ))}
